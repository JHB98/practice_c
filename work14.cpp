...
struct list   // 학생이름과 시도횟수, 점수를 담을 구조체
{
	...
	for (int i = 0;i < peopleNum;i++)
	{
		...
		while (1)
		{
			...
			if ((answerGive + num) < answer) // 정답이 오차범위의 최대값보다 클 때
			{
				...
				answerGive = (rand() % (300 - (answerGive + num))) + ((answerGive + num) + 1); // 제시할 정답 범위를 ***(원래 시도했던 숫자의 오차범위 최대값 +1) ~ 300 *** 으로 설정 후 재 시도
			...
			else if ((answerGive - num) > answer) // 정답이 오차범위의 최소값보다 작을 때
			{
				count++;
				score -= 10;
				answerGive = rand() % (answerGive - num); // 제시할 정답 범위를 *** 0~(원래 시도했던 숫자의 오차범위 최소값-1) *** 으로 설정 후 재시도
				if (range != 4)
				{
					range++;
				}
